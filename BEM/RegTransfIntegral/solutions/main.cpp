...
double integrateTiSing(const Eigen::Vector2d& b, const Eigen::Vector2d& c,
  ...
  for (int k = 0; k < n; k++) {
    res +=
        (std::sqrt(Rtheta(xq(k)) * Rtheta(xq(k)) + zeta * zeta) - zeta) * wq(k);
  }
  return res;
}
/* SAM_LISTING_END_3 */
...
void transformCoordinates(const Eigen::Vector3d& a, const Eigen::Vector3d& b,
  ...
  // the first quadrant. We check whether this is satisfied and flip the
  // triangle if not.
  if (R(0, 0) < 0) {
    R.row(0) *= -1;
  }
...
double integrateTSing(const TriaPanel& T, const Eigen::Vector3d& x, int n = 6) {
  ...
  if (onTria) {
    ...
    for (int i = 0; i < 3; i++) {
      if (fabs(1 - coeff(i)) < tol && !boundary) {
        // Then we are close to vertex i. We don't split the triangle T but move
        // xp to the origin and then integrate.
  ...
  // If xp is not on T
    ...
    // xp outside of T could mean that it is at the boundary of T or in the
    // exterior.
    bool exterior = true;
    ...
    if (exterior) {
      // Get quadrature points and weights for [0, 1] to use tensor-Gauss
      // quadrature
      Eigen::RowVectorXd xq, wq;
      ...
      for (int l1 = 0; l1 < n; l1++) {
        for (int l2 = 0; l2 < n; l2++) {
          ...
          Eigen::Vector2d qp;
          qp << xq(l1), xq(l2) * (1. - xq(l1));
          // Multiply quadrature weight by determinants coming from
          // transformations
          double dy = wq(l1) * wq(l2) * (1. - xq(l1)) *
                      sqrt(fabs((Fk.transpose() * Fk).determinant()));
          // add contribution
