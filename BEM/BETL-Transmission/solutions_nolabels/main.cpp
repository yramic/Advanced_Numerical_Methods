...
Eigen::VectorXd solveTransmissionProblem(const std::string meshname,
  ...
  //============================================================================
  const auto coeff_TDui =
      DofInterpolator()(analytical_TDuinc, dh_lagrange1.fespace());

  const auto coeff_TNui =
      DofInterpolator()(analytical_TNuinc, dh_lagrange0.fespace());
  ...
  typedef IdentityOperator<dh_lagrange0_t::fespace_t, dh_lagrange1_t::fespace_t>
      id_op01_t;
  id_op01_t id_op01(dh_lagrange0.fespace(), dh_lagrange1.fespace());
  id_op01.compute();
  const auto& M = id_op01.matrix();
  // Evaluate <TD0 uinc, v> and <TN0 uinc, v> (taking into account that TN0 =
  // -TN)
  ...
  std::cout << " Energy is : "
            << computeEnergy(sol, gridFactory, dh_lagrange0, dh_lagrange1)
            << std::endl;
