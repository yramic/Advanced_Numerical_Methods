...
#include "discontinuous_bem_space.hpp"
...
namespace parametricbem2d {
  ...
  Eigen::MatrixXd SingleLayer(const AbstractParametrizedCurve& pi,
                              ...
                              const int& p) {
    // The space S_{p}^{-1}
    AbstractBEMSpace space = new DiscontinuousSpace<p>;
  ...
  double ComputeIntegralCoinciding(const AbstractParametrizedCurve& pi,
    ...
    // Lambda expression for the integral 1
    ...
    // Lambda expression for integral 2 in transformed coordinates
    ...
    // Double sum for double integral2
    for ( unsigned int i = 0; i<N ; ++i ) {
  ...
  double ComputeIntegralAdjacent(const AbstractParametrizedCurve& pi,
    ...
    double length_pi = pi.getLength();
    double length_pi_p = pi_p.getLength();
    ...
    auto D_r_phi = [&] (double r,double phi) {
    ...
    for (unsigned int i = 0; i<N ; ++i) {
      // Transforming into phi (alpha to pi/2)
      double phi = points[i]*(M_PI/2.-alpha)/2.+(M_PI/2.+alpha)/2.;
      // Computing log weighted inner integral with fixed phi
      double inner1=0.,inner2=0.;
      double rmax = length_pi_p/sin(phi); // Upper limit for r integral
      for (unsigned int j = 0; j<N ; ++j) {
        // Getting Quadrature weights and nodes for log weight
        QuadRule logweightQR = getLogWeightQR(rmax,N);
        double r = logweightQR.x(j); //for inner2
