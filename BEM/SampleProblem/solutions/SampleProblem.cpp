 ...
 * \param[out] col\_ptr Indices of the elements in 'val' which start a column of
 * $A$
 */
/* SAM_LISTING_BEGIN_0 */
void CCS(const MatrixXd& A, VectorXd& val, VectorXd& row_ind,
         VectorXd& col_ptr) {
...
int main() {
  ...
  // Caste $A$ to 'SparseMatrix' and compress it to store the matrix in CCS
  // format
  ...
  std::cout << "l2-norm of the difference between val = " << sqrt(diff_val)
            << std::endl;
  std::cout << "l2-norm of the difference between row_ind = "
            << sqrt(diff_row_ind) << std::endl;
  std::cout << "l2-norm of the difference between col_ptr = "
            << sqrt(diff_col_ptr) << std::endl;
}
