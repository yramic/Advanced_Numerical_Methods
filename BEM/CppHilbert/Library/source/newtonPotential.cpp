...
void computeN(Eigen::MatrixXd& N,
  ...
  for (int j = 0; j < nT; ++j) { /*  running over triangles */
    nodes << vertices.row(triangles(j, 0)), vertices.row(triangles(j, 1)),
...
//-----------------------------------------------------------------------------
double computeNkjSemiAnalyticTriangle(
  ...
  double detFT = fabs((n2[0] - n1[0]) * (n3[1] - n1[1]) -
                      (n3[0] - n1[0]) * (n2[1] - n1[1]));
  double tmp = 0.;
  ...
  for (int i = 0; i < nr_gauss_points; i++) {
    ...
    Eigen::Vector2d aux2 =
        0.5 * (b + a) - (n2 - n1) * gp_x[i] - (n3 - n1) * gp_y[i] - n1;
...
double computeNkjAnalytic(const Eigen::Vector2d& a, const Eigen::Vector2d& b,
  ...
  double detFK = fabs((n2[0] - n1[0]) * (n3[1] - n1[1]) -
                      (n3[0] - n1[0]) * (n2[1] - n1[1]));
...
double newtonPotential(const Eigen::Matrix<double, 3, 2>& nodes,
  ...
  double volT = 0.5 * fabs((n2[0] - n1[0]) * (n3[1] - n1[1]) -
                           (n3[0] - n1[0]) * (n2[1] - n1[1]));
  const Eigen::Vector2d& u = n1 - n2;
  ...
  double result = volT * integralI + coeff * integralII - coeff * integralIII -
                  volT + coeff * atanint;
...
double integrateAtanInt(const Eigen::Vector2d& a, const Eigen::Vector2d& b,
  ...
  for (int k = 0; k < GAUSS_ORDER; ++k) {
    ...
    const Eigen::Vector2d& sx =
        ((1 - gauss_point[k]) * a + (1 + gauss_point[k]) * b) * 0.5;
