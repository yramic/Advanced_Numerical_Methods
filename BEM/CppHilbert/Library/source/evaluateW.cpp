...
void evaluateW(Eigen::VectorXd& Wx, const BoundaryMesh& mesh,
  ...
  for (int j = 0; j < nX; ++j) {
    ...
    int found_element =
        0;             /* Number of elements containing the evaluation point. */
    ...
    for (int i = 0; i < nE; ++i) {
      ...
      if (dist_x_Ei < 5e-2 * EPS) {
        ...
        if ((a - xj).norm() < EPS || (b - xj).norm() < EPS) {
          fprintf(stderr,
                  "Warning (evaluateW): The evaluation point "
      ...
      if (dist_x_Ei < 5e-2 * EPS) {
        ...
        if ((sqrt(4 * u_sqnorm) > eta * dist_x_Ei) || eta == -1) {
          ...
          if (sqrdelta * sqrdelta > 4.0 * EPS * u_sqnorm * v_sqnorm) {
            g_0m2 = (2 * (u_sqnorm + udotv) / (upvsq)-2 * (-u_sqnorm + udotv) /
                         (umvsq) +
                     2 * u_sqnorm * g_0m1) /
                    (sqrdelta * sqrdelta);
          } else {
            g_0m2 = 2 * (u_sqnorm + 3 * v_sqnorm) /
                    (3 * std::pow(v_sqnorm - u_sqnorm, 3));
          }

          /* g_1^(-2)*/
          g_1m2 =
              (-1 / upvsq + 1 / umvsq - 2 * udotv * g_0m2) / (2. * u_sqnorm);
          ...
          W_0 = (n_vc.dot(n_x.row(j)) * g_0m1 -
                 2 * (u.dot(n_x.row(j)) * n_vc.dot(u) * g_1m2 +
                      (v.dot(n_x.row(j)) * n_vc.dot(u) +
                       u.dot(n_x.row(j)) * n_vc.dot(v)) *
                          g_1m2 +
                      u.dot(n_x.row(j)) * n_vc.dot(v) * g_0m2)) /
                M_PI;
          ...
          W_1 = (n_vc.dot(n_x.row(j)) * g_1m1 -
                 2 * (u.dot(n_x.row(j)) * n_vc.dot(u) * g_2m2 +
                      (v.dot(n_x.row(j)) * n_vc.dot(u) +
                       u.dot(n_x.row(j)) * n_vc.dot(v)) *
                          g_2m2 +
                      v.dot(n_x.row(j)) * n_vc.dot(v) * g_1m2)) /
                M_PI;
        } else {
          ...
          for (int k = 0; k < GAUSS_ORDER; ++k) {
            double denom =
                u_sqnorm * qp[k] * qp[k] + 2 * udotv * qp[k] + v_sqnorm;
            W_0 +=
                qw[k] *
                (n_vc.dot(n_x.row(j)) / denom -
