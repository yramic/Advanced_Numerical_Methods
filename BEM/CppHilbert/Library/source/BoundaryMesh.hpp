...
///                            elements.row(i)=[j, k],
///
/// where the nodes are given in *counterclockwise* order,i.e. the
/// parametrization of the boundary element \f$ E_i \subset \Gamma \f$ is
/// mathematically positive. In other words, the outer normal vector n_i in R^2
/// of Gamma on a boundary element \f$E_i = [z_j, z_k]\f$ reads
///
...
class BoundaryMesh {
 private:
  ...
  /// type for mesh elements (storing indices of the vertices conforming the
  /// given element)
 ...
 public:
   ...
   */
  BoundaryMesh(const std::string& filename) { loadMeshFromFile(filename); }
