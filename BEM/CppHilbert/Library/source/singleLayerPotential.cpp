...
Eigen::VectorXd slpIterative(int k, const Eigen::Vector2d& u,
  ...
  if (fabs(u[0]) < EPS && fabs(u[1]) < EPS) {  // constant integrand
    ...
    val[0] =
        (0.5 * ((b + 2 * a) * log(a + b + c) - (b - 2 * a) * log(a - b + c)) +
         D * val[0]) /
            a -
        4.0;
  }  // \Label[line]{slp:2a}
  ...
  if (k >= 1) {
    if (fabs(u[0]) < EPS && fabs(u[1]) < EPS) {
      ...
      tmp = a + b + c;
      if (fabs(tmp) > EPS * a) val[1] += tmp * log(tmp);
  ...
  for (int i = 2; i <= k; ++i) {
    if (fabs(u[0]) < EPS && fabs(u[1]) < EPS) {
      ...
      tmp = a - b + c;
      if (i % 2 == 0) {
        if (fabs(tmp) > a * EPS) val[i] += tmp * log(tmp);
        val[i] -= 4 * a / (i + 1);
      } else {
        if (fabs(tmp) > a * EPS) val[i] -= tmp * log(tmp);
        val[i] -= 2 * b / i;
...
double computeWijAnalytic(const Eigen::Vector2d& a, const Eigen::Vector2d& b,
  ...
  else {  // \cob{$\Bx$} and \cob{$\By$} linearly independent, Case I
          // \Label[line]{Wij:1}
