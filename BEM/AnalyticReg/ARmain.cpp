...
MatrixXd computeGammaCoefficients(const PARAM& gamma, int N) {
  ...
  MatrixXd TR_points(2 * N, 2);
  double TR_w;
  std::tie(TR_points, TR_w) = PeriodicTrapRule(2 * N);
  // Variable for returning result.
  MatrixXd coeffs(2 * N, 2);
  coeffs.setZero();
  // Matrices for storing cosine and sine coefficients
  MatrixXd cos_coefficients(N, 2);
  cos_coefficients.setZero();
  MatrixXd sin_coefficients(N, 2);
  sin_coefficients.setZero();
...
Vector2d evaluateGammaDiff(const MatrixXd& gammaCoeffs, double s, double t,
                           int N) {
  assert(gammaCoeffs.rows() == 2 * N);
  // Vector for storing sin and cos values in first and second half respectively
  VectorXd sine_values(N);
  sine_values.setZero();
  VectorXd cosine_values(N);
  cosine_values.setZero();
  ...
  Vector2d res = (-(cosine_coefficients).transpose() * sine_values +
 ...
 * \tparam PARAM Type for gamma which supports evaluation operator:
 * gamma(double) \param[in] gamma Function that takes a double and returns a 2d
 * vector corresponding to the parametrized curve. \param[in] N Discretization
 * parameter indicating number of basis functions.
 */
 ...
 * \tparam PARAM Type for gamma which supports evaluation operator:
 * gamma(double) \tparam FUNC Type for function g which supports evaluation
 * operator: g(double) \param[in] gamma Function that takes a double and returns
 * a 2d vector corresponding to the parametrized curve. \param[in] g Right hand
 * side function (takes 2d points and returns a double). \param[in] N
 * Discretization parameter indicating number of basis functions.
 */
/* SAM_LISTING_BEGIN_2 */
 ...
 *                 (2N+2 points).
 * \tparam PARAM Type for gamma which supports evaluation operator:
 * gamma(double) \tparam PARAMDER Type for gammaprime which supports evaluation
 * operator: gammaprime(double) \param[in] coeffs coefficients of UN. \param[in]
 * X 2d vector containing evaluation point. \param[in] gamma Function that takes
 * a double and returns a 2d vector corresponding to the parametrized curve.
 * \param[in] gammaprime Function that takes a double and returns a 2d vector
 *                       corresponding to the derivative of the curve's
 *                       parametrization.
 */
...
double repFormulaSL(const VectorXd& mu, const Vector2d& X, const PARAM& gamma,
                    const PARAMDER& gammaprime) {
  ...
  for (int qp = 0; qp < Nq; qp++) {
    ...
    res += -TR_w / (2 * M_PI) * log((X - gamma(z)).norm()) * rho *
           (gammaprime(z)).norm();
  }
  return res;
}
...
/* TODO: You may test your quadrature by integrating 1, cos(t) and
...
#else   // TEMPLATE
  // TODO: You may test your computation of the coefficients for gamma(t) and
  // the difference (gamma(0.1)-gamma(0))/||S_hat(0.1)-S_hat(0)||.
  Vector2d diffS;
  diffS.setZero();
#endif  // TEMPLATE
  // Calculating gamma(s)-gamma(t) exactly
  Vector2d exDiffS;
  exDiffS << cos(0.1) - cos(0), sin(0.1);
  std::cout << "(S(0.1)-S(0))/||S(0.1)-S(0)|| = " << std::endl;
  std::cout << "Calculated" << std::setw(20) << "Exact" << std::endl;
  std::cout << diffS.transpose()(0) << ", " << diffS.transpose()(1)
            << std::setw(16)
            // Calculating (gamma(s)-gamma(t))/||S_hat(s)-S_hat(t)|| ; gamma = S
            // = S_hat
            << exDiffS(0) / exDiffS.norm() << ", "
            << exDiffS(1) / exDiffS.norm() << std::endl;
  std::cout << "=============================================================="
            << "============" << std::endl
            << std::endl;
  ...
  std::cout << "ERROR for evaluating at PI: "
            << fabs(solCEval - gC(S(M_PI)) * 2) << std::endl;
  std::cout << "====================================================="
            << std::endl
            << std::endl;
#endif  // SOLUTION
  ...
  //----------------------------------------------------------------------------
  /* SAM_LISTING_BEGIN_6 */
  std::cout << "=====  Test system for gamma(t)  =====" << std::endl;
