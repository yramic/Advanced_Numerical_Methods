...
int main(int argc, char* argv[]) {
  ...
  // Define bases for $\qbe, \sbe$ and edge functions (to be used when
  // implementing the Hypersingular operator using integration by parts formula
  // (1.3.92))
  ...
  typedef bem::GalerkinKernel<laplace_fs_t, bem::FSLayer::SL, feb_lagr0_t,
                              feb_lagr0_t>
      lagr_sl_kernel_t;
  // - Kernel for K: Double layer with $\qbe, \sbe$ bases (we get K' by
  // transposing)
  typedef bem::GalerkinKernel<laplace_fs_t, bem::FSLayer::DL, feb_lagr0_t,
                              feb_lagr1_t>
      lagr_dl_kernel_t;
  // - Kernel for W using integration by parts
  typedef bem::GalerkinKernel<laplace_fs_t, bem::FSLayer::SL, feb_div_t,
                              feb_div_t>
      div_sl_kernel_t;
  ...
  //============================================================================
  // Define the singularity detector in order to identify the different
  // singularity situations on the given grid (as discussed in section 1.5.3:
  // coinciding panels, adjacent panels, common vertex and panels with positive
  // distance).
  typedef bem::GalerkinSingularityDetector<grid_factory_t>
  ...
  // DEFINE QUADRATURE
  //============================================================================
  // Define the quadrature rule by setting number of points for each case
  // (positive distance and singular cases)
  ...
  // true. It indicates whether the gram determinants are to be included into
  // the integral or not. In general, these determinants are mandatory. However,
  // certain linear combinations of scalar potentials might be used to express
  // potentials related to edge functions. In that particular case, the gram
  // determinants cancel out and should(must!) be neglected (but this is not the
  // case here, so set it true).
  ...
  typedef bem::GalerkinIntegrator<lagr_sl_kernel_t, integrationTraits>
      lagr_sl_integrator_t;
  ...
  // 'integrationTraits' has no effect since integrators for edge-functions
  // neglect the Gram determinants in any case.
  typedef bem::GalerkinIntegrator<div_sl_kernel_t, integrationTraits>
      div_sl_integrator_t;

  sec_timer_t timer_int;
  lagr_sl_integrator_t lagr_sl_integrator(lagr_sl_kernel, singularity_detector,
                                          cache);
  lagr_dl_integrator_t lagr_dl_integrator(lagr_dl_kernel, singularity_detector,
                                          cache);
  div_sl_integrator_t div_sl_integrator(div_sl_kernel, singularity_detector,
                                        cache);
  ...
  // required FE-spaces. We retrieve these spaces from the dofhandler
  // constructed for each one of them.

  // - V
  typedef BemOperator<lagr_sl_integrator_t, typename dh_lagrange0_t::fespace_t>
      bem_op_V_t;
  ...
  bem_op_K.compute();
  std::cout
      << "Computation of <K psi, phi>, phi in H^{-1/2}, psi in H^{1/2} took: "
      << timer_op_K << std::endl;
  ...
  typedef BemOperator<div_sl_integrator_t, typename dh_div_t::fespace_t>
      bem_op_div_t;
