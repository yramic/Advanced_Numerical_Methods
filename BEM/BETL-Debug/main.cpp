...
//------------------------------------------------------------------------------
void computeResiduals(const grid_factory_t& gridFactory,
  ...
  // COMPUTE RESIDUALS ACCORDING TO (1.6.33)
  // Residual for Dirichlet
  const Eigen::VectorXd res_D = 0.5*M *coeff_gD +  K * coeff_gD - V * coeff_gN;

  // Residual for Neumann 
  const Eigen::VectorXd res_N = -W * coeff_gD + 0.5*M.transpose()* coeff_gN
                                  - K.transpose() * coeff_gN ;

  // Output Euclidean norm of residuals
  std::cout << "Euclidean norm of Dirichlet residuum = " << ( res_D ).norm( )
	    << std::endl;
  std::cout << "Euclidean norm of Neumann residuum = " << ( res_N ).norm( )
	    << std::endl;
}
...
int main( int argc, char* argv[] )
{
  ...
  for(int k=0; k<4; k++){
    ...
    // RESIDUALS
    //============================================================================
    computeResiduals(gridFactory, dh_lagrange0, dh_lagrange1, dh_div, laplace_fs);
