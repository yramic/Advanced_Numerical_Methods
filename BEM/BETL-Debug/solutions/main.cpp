...
void debugV(const matrix_t& V) {
  ...
  if (DV.real().minCoeff() < 0) {
    std::cout << " V has a negative eigenvalue! " << DV.real().minCoeff()
              << std::endl;
  } else {
...
void debugW(const matrix_t& W) {
  ...
  if (DW.real().minCoeff() < -1e-12) {
    std::cout << " W has a negative eigenvalue! " << DW.real().minCoeff()
              << std::endl;
  } else {
...
Eigen::VectorXd computeDirichletResidual(const grid_factory_t& gridFactory,
  ...
  const Eigen::VectorXd res_D =
      0.5 * M * coeff_gD + K * coeff_gD - V * coeff_gN;
...
Eigen::VectorXd computeNeumannResidual(const grid_factory_t& gridFactory,
  ...
  const Eigen::VectorXd res_N =
      -W * coeff_gD + 0.5 * M.transpose() * coeff_gN - K.transpose() * coeff_gN;
...
int main(int argc, char* argv[]) {
  ...
  for (int k = 0; k < 4; k++) {
    ...
    // RESIDUALS
    //============================================================================
    const auto rD = computeDirichletResidual(gridFactory, dh_lagrange0,
                                             dh_lagrange1, laplace_fs);

    const auto rN = computeNeumannResidual(gridFactory, dh_lagrange0,
                                           dh_lagrange1, dh_div, laplace_fs);
