...
#ifndef ABELINTEGRALEQUATION_UTILITIES_H_
#define ABELINTEGRALEQUATION_UTILITIES_H_
...
MatrixXd toeplitz(const VectorXd& c, const VectorXd& r) {
  if (c(0) != r(0)) {
    cerr << "First entries of c and r are different!" << endl
         << "We assign the first entry of c to the diagonal" << endl;
  }
 ...
 */
VectorXd toepMatVecMult(const VectorXd& c, const VectorXd& r,
                        const VectorXd& x) {
...
void test_approx_eq_ieul_abel() {
  ...
  cout << "\n\nConvolution Quadrature, Implicit-Euler, Function approximation "
          "test\n"
       << endl;
  for (int N = 16; N <= 4906; N *= 2) {
    ...
    cout << "N = " << N << setw(15) << "Max = " << scientific << setprecision(3)
         << err_max << endl;
  }
}
