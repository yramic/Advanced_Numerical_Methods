...
VectorXd conv_wght_ieu(const FFUNC& F, size_t n, int p, double r=1.0E-7)
{
    ...
    for(int i=0; i<p; ++i) {
        ...
        complex<double> s = r * complex<double>(cos(2.*M_PI*i/p), sin(2.*M_PI*i/p));
...
VectorXd conv_wght_bdf2(const FFUNC& F, size_t n, int p, double r=1.0E-7)
{
    ...
    // define integrand after transformation: $s = r \exp(2 \pi \varphi)$, where parameter is $\varphi$    
    auto integrand = [](const FFUNC& F, double tau, complex<double> s, int l)
    { return s/sqrt(1.+2.*tau*s) * (1./pow(2.+sqrt(1.+2.*tau*s),l+1) - 1./pow(2.-sqrt(1.+2.*tau*s),l+1)) * F(s); };
...
SparseMatrix<double> compute_matA(size_t N)
{
    ...
    A.insert(0,0) = 1./h + 1./(h*h*pow(M_PI,3))*((pow(M_PI*h,2)-2.) + 2.*cos(M_PI*h)); //A(0,0)
    A.insert(N,N) = 1./h + 1./(h*h*pow(M_PI,3))*((pow(M_PI*h,2)-2.) - 2.*cos(M_PI*(1-h))); //A(N,N)
    
    for(int i=1; i<=N; ++i) {
        if (i<N) { // A(i,i)
            A.insert(i,  i) = 2./h + 2./(h*h*pow(M_PI,3))*(2*M_PI*h*sin(M_PI*grid(i)) + cos(M_PI*grid(i+1)) - cos(M_PI*grid(i-1)));
        }
        // A(i-1,i) = A(i,i-1)
        A.insert(i-1,i) = A.insert(i,i-1) =-1./h + 1./(h*h*pow(M_PI,3))*(2.*(cos(M_PI*grid(i-1)) - cos(M_PI*grid(i))) - M_PI*h*(sin(M_PI*grid(i-1)) + sin(M_PI*grid(i))));
    }
...
VectorXd solve_IBVP(const FUNC& g, size_t M, size_t N, int p)
{
    ...
    for(int i=1; i<=M; ++i) {
        ...
        VectorXcd rhs = phi; rhs(N) -= rhs_cq; //rhs
...
int main() {
    ...
    for(int N=16; N<=N_ref/4; N*=2) 
    {
        ...
        double ratio = N_ref/N;
          for(int i=1; i<=N_ref; ++i) {
            int j = ceil(i/ratio);
            error += pow((u_ref(i) - u_ref(i-1))/h_ref - (u_tmp(j) - u_tmp(j-1))/h, 2);
        }
