...
void Node::setSons(int& id) {
  if (node_points_.size() > 1) {
    ...
    l_child_ =
        new Node(lc, id, deg_);  // recursivly do the same for the left child
    id++;  // increase the identification number of the nodes of the tree
    r_child_ =
        new Node(rc, id, deg_);  // recursivly do the same for the right child
    double xmin =
...
void Node::resetCVc() { CVc_node_.setZero(); }
