...
// constructor
template <>
LowRankApp<BlockCluster_Y, Node_Y>::LowRankApp(
    Kernel *kernel, const std::vector<Point> &GPoints, double eta, unsigned deg)
    : kernel_(kernel), GPoints_(GPoints), HP_(GPoints, eta, deg), deg_(deg),
      nops_(0), debug_(false) {
...
LowRankApp<BlockCluster_Y, Node_Y>::LowRankApp(
    ...
    : kernel_(kernel), GPoints_(GPoints), HP_(GPoints, eta, deg), deg_(deg),
      nops_(0), debug_(true), myfile_(filename.c_str(), std::ios::app) {
  // Recursive hierarchical partitioning to get Near/Far field boxes
  HP_.setNearFar();
}
...
template <>
void LowRankApp<BlockCluster_Y, Node_Y>::preProcess(std::vector<Node *> ff_v_x,
...
template <>
void LowRankApp<BlockCluster_Y, Node_Y>::blockProcess(
...
template <>
void LowRankApp<BlockCluster_Y, Node_Y>::blockProcessClear(
    std::vector<BlockCluster *> ff_v) {
  for (auto &pair : ff_v) { // iterate for all the pairs of far field nodes
...
template <>
void LowRankApp<BlockCluster_Y, Node_Y>::debugProcess(
    std::vector<BlockCluster *> ff_v) {
...
template <>
void LowRankApp<BlockCluster_Y, Node_Y>::postProcess(std::vector<Node *> ff_v_x,
                                                     Eigen::VectorXd &f) {
  for (auto &xnode : ff_v_x) { // iterate for all the far field xnodes
...
// matrix
template <>
void LowRankApp<BlockCluster_Y, Node_Y>::calc_numb_approx_per_row(
...
template <>
void LowRankApp<BlockCluster_Y, Node_Y>::ff_contribution(
...
template <>
void LowRankApp<BlockCluster_Y, Node_Y>::nf_contribution(
...
template <>
Eigen::VectorXd
LowRankApp<BlockCluster_Y, Node_Y>::mvProd(const Eigen::VectorXd &c) {
