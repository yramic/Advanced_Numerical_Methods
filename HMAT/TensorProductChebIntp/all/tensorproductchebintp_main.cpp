...
int main(int /*argc*/, char** /*argv*/) {
  auto kernel1D = [](double x) {
    return 1. / (1. + (x * x));
  };  // the 1d kernel we want to interpolate
  auto kernel2D = [](double x, double y) {
    return 1. / (1. + (x - y) * (x - y));
  };  // the 2d kernel we want to interpolate
  ...
  for (int n = 0; n < N; ++n) y[n] = distr(generator);
  ...
  for (int q = 1; q < 10; q++) {
    const std::vector<double> res =
        TensorProductChebIntp::chebInterpEval1D(q, kernel1D, y);
  ...
  for (int q = 3; q < 15; q++) {
    const std::vector<double> res =
        TensorProductChebIntp::chebInterpEval2D(q, kernel2D, x);
  ...
  for (int q = 3; q < 15; q++) {
    const std::vector<double> res2 =
        TensorProductChebIntp::genChebInterpEval2D(q, kernel2D, a, b, x2);
