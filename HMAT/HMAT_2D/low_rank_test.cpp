...
//    unsigned q = 2;
    ...
    KernelGalerkin G(1.); // initialization of Galerkin kernel for 2d problem -1/(2*pi)*log||x-y||
    ...
    for(int i=0; i<n; ++i)
        for(int j=0; j<n; ++j)
            M(i,j) = G(points[i].getX(), points[i].getY(), points[j].getX(), points[j].getY());
    ...
    LowRankApp HMat(&G, points, eta, q); // initialization of low rank approximation for BEM approx for matrix multiplication
    ...
    std::cout << "Approximation error between f_exact and f_approx (l-inf norm of vector diff): " << diff_g.lpNorm<Eigen::Infinity>() << std::endl
              ...
              << "Relative Approximation error between f_exact and f_approx (l-2 norm of diff/l-2 norm of f_exact): "   << diff_g.lpNorm<2>()/f_exact.lpNorm<2>() << std::endl
              << "Time needed for exact multiplication: "       << time_diff1.count() << " s" << std::endl
