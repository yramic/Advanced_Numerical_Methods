...
LowRankApp::LowRankApp(Kernel* kernel, const std::vector<Point>& pp, double eta,
                       unsigned deg)
...
void LowRankApp::debugProcess(std::vector<BlockCluster*> ff_v) {
  ...
  myfile_ << "error_Frobenius, " << points_.size() << ", "
          << std::setprecision(10) << error_Frobenius / ff_v.size()
          << std::endl;  // average error w.r.t. all blocks
  myfile_ << "error_max, " << points_.size() << ", " << std::setprecision(10)
          << error_max << std::endl;
}
...
void LowRankApp::calc_numb_approx_per_row(std::vector<BlockCluster*> ff_v,
  ...
  for (auto& pair : ff_v) {  // iterate for all the pairs of far field nodes
    ...
    for (int i = 0; i < xnode->getPPoints().size(); i++) {
      f_approx_ff_contr(xnode->getPPoints()[i].getId()) +=
          ynode->getPPoints().size();
    }
  }
}
