...
double KernelPolynomial::operator()(double x1, double y1, double x2,
                                    double y2) {
...
double KernelGlobalSmooth::operator()(double x1, double y1, double x2,
                                      double y2) {
  return num_ *
         std::cos(std::sqrt(std::pow(x1 - x2, 2) + std::pow(y1 - y2, 2)));
}
...
double KernelGauss::operator()(double x1, double y1, double x2, double y2) {
  ...
  return num_ *
         std::exp(-(std::pow(
             (std::sqrt(std::pow(x1 - x2, 2) + std::pow(y1 - y2, 2))), 2)));
}
