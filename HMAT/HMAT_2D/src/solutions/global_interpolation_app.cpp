...
GlobalInterpolationApp::GlobalInterpolationApp(Kernel* kernel,
                                               const std::vector<Point>& pp)
    : K_(kernel), PPoints_(pp) {}

// approximate matrix-vector multiplication
Eigen::VectorXd GlobalInterpolationApp::mvProd(Eigen::VectorXd& c,
                                               unsigned deg) {
  ...
  for (std::vector<Point>::iterator it = PPoints_.begin() + 1;
       it != PPoints_.end(); it++) {
    if (it->getX() > maxX) {
      maxX = it->getX();
  ...
  for (int i = 0; i <= deg; i++) {
    for (int j = 0; j <= deg; j++) {
      for (int k = 0; k <= deg; k++) {
        for (int l = 0; l <= deg; l++) {
          X(i * (deg + 1) + j, k * (deg + 1) + l) =
              (*K_)(cbx.getNodes()[i], cby.getNodes()[j], cbx.getNodes()[k],
                    cby.getNodes()[l]);
        }
      }
    }
