...
std::pair<Eigen::MatrixXd, Eigen::MatrixXd> low_rank_merge(
  ...
  Eigen::MatrixXd S;
  S.setZero(A1.cols(), A1.cols());  // only consider first q singular values
  S.diagonal() = s.head(A1.cols());
  Eigen::MatrixXd U =
      SVD.matrixU().leftCols(A1.cols());  // only consider first q columns
  ...
  Eigen::MatrixXd Q = Eigen::MatrixXd::Zero(
      Q1.rows() + Q2.rows(), Q1.cols() + Q2.cols());  // (2.4.2.26)
...
std::pair<double, double> test_low_rank_merge(size_t n) {
  ...
  std::pair<Eigen::MatrixXd, Eigen::MatrixXd> AB = low_rank_merge(
      A1, B1, A2, B2);  // Call the function implemented in sub-problem (2-6.b)
...
std::pair<Eigen::MatrixXd, Eigen::MatrixXd> adap_rank_merge(
  ...
  // Only consider singular values larger than rtol * largest singular value as
  // in (2.4.2.20)
  unsigned p = s.size();
  ...
  Eigen::MatrixXd Q =
      Eigen::MatrixXd::Zero(Q1.rows() + Q2.rows(), Q1.cols() + Q2.cols());
  Q.block(0, 0, Q1.rows(), Q1.cols()) = Q1;
  Q.block(Q1.rows(), Q1.cols(), Q2.rows(), Q2.cols()) = Q2;
...
std::pair<double, size_t> test_adap_rank_merge(size_t n, double rtol) {
  ...
  std::pair<Eigen::MatrixXd, Eigen::MatrixXd> AB = adap_rank_merge(
      A1, B1, A2, B2, rtol);  // Call the function from sub-problem (2-6.d)
  Eigen::MatrixXd Ztilde = AB.first * AB.second.transpose();
