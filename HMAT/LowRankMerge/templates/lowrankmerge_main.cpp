...
int main(int argc, char **argv) {
  ...
  for (int p = 3; p <= 12; ++p) {
    ...
    std::cout << "n = " << n << std::setw(15)
              << "Frobenius = " << std::scientific << std::setprecision(3)
              << errs.first << std::setw(10) << "Max = " << std::scientific
              << std::setprecision(3) << errs.second << std::endl;
  }
  ...
  for (int p = 3; p <= 12; ++p) {
    unsigned n = std::pow(2, p);
    std::pair<double, size_t> errs =
        LowRankMerge::test_adap_rank_merge(n, rtol);
    std::cout << "n = " << n << std::setw(15)
              << "Frobenius = " << std::scientific << std::setprecision(3)
              << errs.first << std::setw(10) << "Rank = " << errs.second
              << std::endl;
  }
  std::cout << "Problem 2-6.e" << std::endl;
