...
int main() {
    ...
    unsigned n=16;  // initalizing n for testing
    ...
    for(int i=0; i<n; ++i)
        for(int j=0; j<n; ++j)
            ...
            if(i!=j) M(i,j) = P(PPoints[i].getX(), PPoints[i].getY(), PPoints[j].getX(), PPoints[j].getY());    // the diagonal of the matrix will be zero because the 4D kernel between the same point is inf
