...
class LowRankApp {
 public:
   ...
   */
  LowRankApp(Kernel* kernel, const std::vector<Segment>& segments, double eta,
             unsigned deg);
  /*!
   * \brief Constructor for 2D Low Rank Approximation
   * \param kernel Kernel used for the matrix multiplication
  ...
  /*!
   * \brief Count number of corresponding far field points for each row of the
   * product vector \param ff_v Vector of BlockClusters \param f_approx_ff_contr
   * Vector for saving the number of corresponding far field points for each row
 ...
 private:
  ...
  void ff_contribution(std::vector<BlockCluster*> ff_v,
                       ...
                       Eigen::VectorXd& f_aprox_ff_contr);
  /*!
  ...
  void nf_contribution(std::vector<BlockNearF*> nf_v, const Eigen::VectorXd& c,
                       Eigen::VectorXd& f, Eigen::VectorXd& f_aprox_nf_contr);
