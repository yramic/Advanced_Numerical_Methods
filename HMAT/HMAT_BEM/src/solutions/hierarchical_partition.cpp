...
HierarchicalPartitioning::HierarchicalPartitioning(
    const std::vector<Segment>& segments, double eta, unsigned deg)
...
void HierarchicalPartitioning::setNearFar_recursion(Node* xnode, Node* ynode,
                                                    double eta) {
  ...
  AdmissibilityH adm;
  /* SAM_LISTING_BEGIN_0 */
  if ((*ynode).getSegments().size() <= 1 ||
      (*xnode).getSegments().size() <= 1) {
...
HierarchicalPartitioning::getBB(int i) {
  Node* xnode = FarField_[i]->getXNode();
  Node* ynode = FarField_[i]->getYNode();
  return {{{xnode->getX1(), xnode->getY1()}, {xnode->getX2(), xnode->getY2()}},
          {{ynode->getX1(), ynode->getY1()}, {ynode->getX2(), ynode->getY2()}}};
}
