#! /bin/bash
# Creates mysolution and templates from all-in-one C++ code
...
for dir in $@; do
  ...
  # problems
  cp -r $output_dir/mysolution/. $output_dir/mysolution
  # Strip template parts 
  unifdef -DSOLUTION=1 -x 2 -m $output_dir/mysolution/*.*
  # Strip solution 
  unifdef -DSOLUTION=0 -x 2 -m $output_dir/mysolution/*.*

  # tests
  if [ -d "$output_dir/mastersolution/test" ]; then
    unifdef -DSOLUTION=1 -x 2 -m $output_dir/mysolution/test/*.*
