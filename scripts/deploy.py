...
def mkdir(dir):
    """
    Creates a new directory, if it exists it will be removed and replaced by an empty one.
    """
...
def generate_templates_and_mysolution(problem_dir, problem_dir_source):
    ...
    then copy the files in "mysolution" in problem_dir_source to them
    (remove "#if SOLUTION" block, that is, -DSOLUTION=0).
    Delete and recreate "mysolution", copy files with -DSOLUTION=1
    ...
    if not os.path.exists(problem_dir_source + "all/test/"):
        print("There is no unit test in {}".format(problem_dir_source))

    recursive_copy_and_replace(problem_dir_source + 'all/', problem_dir + 'templates/', False)
    recursive_copy_and_replace(problem_dir_source + 'all/', problem_dir + 'mysolution/', False)
    recursive_copy_and_replace(problem_dir_source + 'all/', problem_dir + 'mastersolution/', True)
...
def parse_json(filename):
    ...
    homework_dir = obj["source_dir"]  # "../homeworks/"
    ...
    for problem in obj["Problems"]:
        ...
        problem_dir = source_dir + problem  # "../homeworks/problem_name/"
