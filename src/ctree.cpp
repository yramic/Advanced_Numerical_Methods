...
ctree::ctree(std::vector<double> Xvec) : root(NULL), X(Xvec){	
  int N=Xvec.size();
...
void ctree::V_recursion(node* cluster, int polynom_degree){
  if((*cluster).lchild!=NULL){
    (*cluster).fill_V(X,polynom_degree);
...
void ctree::divide_tree(node* xnode, node* ynode, double eta, ctree Ty){
  ...
  if((*xnode).lchild==NULL || (*ynode).lchild==NULL){
    ...
    if(is_admissible(X[(*xnode).l_ind],X[(*xnode).r_ind],(Ty.get_X())[(*ynode).l_ind],(Ty.get_X())[(*ynode).r_ind],eta)){
...
void ctree::rec_fflist(std::vector<double>& xlist, std::vector<double>& ylist, node* xnode){
  ...
  for(std::vector<node*>::iterator iter=ffx.begin(); iter!=ffx.end();++iter){	
    ...
    xlist.push_back(X[ixl]);  // add the coordinate to the list
    xlist.push_back(X[ixr]); // gaensefuessli
    ylist.push_back(X[iyl]); // gaensefuessli
    ylist.push_back(X[iyr]); // nomal gaensefuessli:-)
  };
