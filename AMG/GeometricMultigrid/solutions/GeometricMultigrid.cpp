...
SparseMatrix<double> genProlongationMat(const int l) 
{
    ...
    for (unsigned k = 0; k < nh + 1; ++k) // k represents the cell row from the base of the triangle
    {
        for (unsigned i = 0; i < nh + 1 - k; ++i) // i represents the local cell index in the cell row number k
        {
            unsigned idh = (nh + 1) * k - (k - 1) * k / 2 + i; // node id for fine mesh
            ...
            if (k % 2 == 0 && i % 2 == 0)
            {
                ...
                unsigned idH = (nH + 1) * kc - (kc - 1) * kc / 2 + ic; // node id for coarse mesh
            ...
            { 
                ...
                unsigned icl = (i - 1) / 2; // cell index value of left point for coarse mesh
                unsigned icr = (i + 1) / 2; // cell index value for right point coarse mesh
            ...
            if (k % 2 == 1 && i % 2 == 0) 
            { 
                unsigned kct = (k + 1) / 2; // cell row value for the upper row for coarse mesh
                unsigned kcb = (k - 1) / 2;     // cell row value for the lower row for coarse mesh
            ...
            if (k % 2 == 1 && i % 2 == 1)
            {
                unsigned kct = (k + 1) / 2; // cell row value for the upper row for coarse mesh
                unsigned kcb = (k - 1) / 2; // cell row value for the lower row for coarse mesh
                unsigned ic = (i - 1) / 2; // cell index value for upper row of coarse mesh
 ...
 */
void gaussSeidel(const SparseMatrix<double> &A, const VectorXd &phi, VectorXd &mu, int maxItr, double TOL = 1.0E-06)
{
